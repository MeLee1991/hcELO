...
export function betterTier(tierA: TierName, tierB: TierName): TierName {
  return getTierIndex(tierA) <= getTierIndex(tierB) ? tierA : tierB;
}

export function enforceMinimumTier(suggestedTier: TierName, minimumTier?: TierName): TierName {
  if (!minimumTier) return suggestedTier;
  return betterTier(suggestedTier, minimumTier);
}
